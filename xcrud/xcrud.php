...
class Xcrud
{
    ...
    protected $grid_condition = array(); // ***** remocve *****
    ...
    public static function get_requested_instance()
    {
        ...
        if (isset($_SESSION['xcrud_session'][$inst_name]['key']) && $_SESSION['xcrud_session'][$inst_name]['key'] == $key)
        {
            ...
            self::error('<strong>Something Went Wrong. Just Reload a Page</strong>');
    }
    ...
    public static function session_start($sess_name = false)
    {
        ...
        if (Xcrud_config::$alt_session)
        {
            if (!headers_sent())
            {
                ...
                {
                    $sess_id = base_convert(str_replace(' ', '', microtime()) . rand(), 10, 36);
                }
                else
                {
                    $sess_id = $_COOKIE[$sess_name];
                }
                setcookie($sess_name, $sess_id, time() + Xcrud_config::$alt_lifetime * 60, '/');
            }
            else
    ...
    public function where($fields = '', $where_val = false, $glue = 'AND', $index = false)
    {
    ...
    /** !!! not available */
    public function fk_relation($fields = '', $rel_tbl = '', $rel_field = '', $rel_name = '', $rel_where = array(), $fk_table =
        '', $in_fk_fiend = '', $out_fk_field = '')
    {
        if ($fields && $rel_tbl && $rel_field && $rel_name)
        {
            ...
            foreach ($fdata as $fitem)
            {
                $this->fk_join[$fitem['table'] . '.' . $fitem['field']] = array(
                    'table' => $fitem['table'],
                    'field' => $fitem['field'],
                    'rel_tbl' => $rel_tbl,
                    'rel_alias' => 'alias' . rand(),
    ...
    public function readonly($fields = '', $mode = 'all') // needs to be re-written
    {
    ...
    public function disabled($fields = '', $mode = 'all')
    {
    ...
    public function condition($fields = '', $operator = '', $value = '', $method = '', $params = array())
    {
    ...
    public function column_cut($int = 50, $fields = false)
    {
        ...
        else
        {
            ...
            foreach ($fdata as $fitem)
            {
                $this->column_cut_list[$fitem['table'] . '.' . $fitem['field']] = $int;
            }
        }
        return $this;
    }
    ...
    public function subselect($column_name = '', $sql = '', $before = false)
    {
        if ($column_name && $sql)
        {
            ...
            $this->field_type[$table . '.' . $column_name] = 'none';
            $this->defaults[$table . '.' . $column_name] = '';
        }
        return $this;
    }
    ...
    public function search_columns($fields = '', $default = null)
    {
        if ($fields)
        {
            ...
            foreach ($fdata as $fitem)
            {
                $this->search_columns[$fitem['table'] . '.' . $fitem['field']] = $fitem;
            }
        }
        if ($default)
        {
    ...
    public function column_pattern($fields, $patern)
    {
        if ($fields && $patern)
        {
            ...
            foreach ($fdata as $fitem)
            {
                ...
                //    '}', $patern);
                $this->column_pattern[$fitem['table'] . '.' . $fitem['field']] = $patern;
            }
        }
        return $this;
    }
    public function column_tooltip($fields = '', $tooltip = '', $icon = false)
    {
        if ($fields && $tooltip)
        {
            ...
            foreach ($fdata as $fitem)
            {
                $this->column_tooltip[$fitem['table'] . '.' . $fitem['field']] = array('tooltip' => $this->html_safe(isset($fitem['value']) ?
                        $fitem['value'] : $tooltip), 'icon' => $icon);
            }
    ...
    protected function _run_task()
    {
        ...
        switch ($this->task)
        {
            ...
            case 'print':
                ...
                break;
            case 'depend':
                return $this->create_relation($this->_post('name', false, 'base64'), $this->_post('value'), $this->_post('dependval'));
                break;
            case 'view':
    ...
    protected function _post($field = '', $default = false, $filter = false)
    {
        ...
        {
            if ($field == 'postdata' && $_POST['xcrud']['postdata'])
            {
                $postdata_keys = array_keys($_POST['xcrud']['postdata']);
                foreach ($postdata_keys as $k => $key)
                {
                    $postdata_keys[$k] = str_replace('`', '', base64_decode(str_replace(array('-', '_'), array('=', '/'), $key)));
                }
                return array_combine($postdata_keys, $_POST['xcrud']['postdata']);
            }
            elseif ($filter)
            {
                switch ($filter)
                {
                    case 'key':
                        return str_replace('`', '', $_POST['xcrud'][$field]);
                        break;
                    case 'int':
                        return (int)$_POST['xcrud'][$field];
                        break;
                    case 'trim':
                        return trim($_POST['xcrud'][$field]);
                        break;
                    case 'base64':
                        return str_replace('`', '', base64_decode(str_replace(array('-', '_'), array('=', '/'), $_POST['xcrud'][$field])));
                        break;
                    default:
                        return $_POST['xcrud'][$field];
                        break;
                }
            }
            else
    ...
    protected function _get($field = '', $default = false, $filter = false)
    {
        if (isset($_GET['xcrud'][$field]))
        {
            if ($filter)
            {
                ...
                {
                    case 'key':
                        return str_replace('`', '', $_GET['xcrud'][$field]);
                        break;
                    case 'int':
                        return (int)$_GET['xcrud'][$field];
                        break;
                    case 'trim':
                        return trim($_GET['xcrud'][$field]);
                        break;
                    default:
                        return $_GET['xcrud'][$field];
                        break;
                }
            }
            else
    ...
    protected function _entry($mode = 'edit', $postdata = array())
    {
        ...
        $this->result_row = array_merge($db->row(), $postdata);
        ...
        if ($this->condition)
        {
            foreach ($this->condition as $field => $params)
            {
                ...
                if (isset($this->result_row[$field]) && $this->_compare($this->result_row[$field], $params['operator'], $params['value']))
                {
    ...
    protected function _update($postdata, $primary)
    {
        ...
        {
            if (!$this->demo_mode)
                $res = $db->query("UPDATE `{$this->table}` SET " . implode(',', $set) . " WHERE `{$this->primary_key}` = " . $db->
    ...
    protected function _remove()
    {
        ...
        if ($this->before_remove)
        {
            ...
            {
                call_user_func_array($this->before_remove['callable'], array($this->primary_val));
            }
        }
    ...
    /** defines primary condition for internan usage */
    protected function where_pri($fields = '', $where_val = '', $glue = 'AND', $index = false)
    {
    ...
    protected function _build_select_list($csv = false)
    {
        ...
        foreach ($this->columns as $key => $val)
        {
            if ($val)
            {
                $field_index = $val['table'] . '.' . $val['field'];
                ...
                elseif (isset($this->join[$field_index]))
                {
        ...
        {
            foreach ($this->hidden_columns as $key => $val)
            {
                $columns[] = "`{$val['table']}`.`{$val['field']}` AS `{$key}` \r\n";
            }
        }
    ...
    protected function subselect_callback($matches)
    {
        if (strpos($matches[1], '.'))
        {
            ...
            if (isset($this->subselect[$tmp[0] . '.' . $tmp[1]]))
            {
                return $this->subselect_where($tmp[0] . '.' . $tmp[1]);
            }
            else
                return '`' . $tmp[0] . '`.`' . $tmp[1] . '`';
        }
    ...
    protected function _build_where()
    {
        ...
        if ($this->search && ($this->task == 'list' or $this->task == 'print' or $this->task == 'csv' or $this->after == 'list'))
        {
            ...
            {
                // if relation
                if (isset($this->join[$this->column]))
                {
                    $where_arr[] = $this->_build_relation_subwhere();
                }
                // search in subselect
                ...
                else
                {
                    ...
                    switch ($this->field_type[$this->column])
                    {
                        ...
                        case 'time':
                            ...
                            {
                                $where_arr[] = '(`' . $fitem['table'] . '`.`' . $fitem['field'] . '` BETWEEN ' . $db->escape(date($format, (int)$this->
                                    phrase['from'])) . ' AND ' . $db->escape(date($format, (int)$this->phrase['to'])) . ')';
                            }
                            elseif ($this->phrase['from'])
                            {
                                $where_arr[] = '(`' . $fitem['table'] . '`.`' . $fitem['field'] . '` >= ' . $db->escape(date($format, (int)$this->
                                    phrase['from'])) . ')';
                            }
            ...
            else
            {
                ...
                foreach ($this->search_columns as $key => $fitem)
                {
                    ...
                    {
                        $or_array[] = $this->_build_relation_subwhere();
                    }
    ...
    protected function _build_relation_subwhere() // multicolumn name
    {
        $db = Xcrud_db::get_instance($this->connection);

        if ($this->column)
        {
            if (is_array($this->join[$this->column]['rel_name']))
            {
                $tmp_fields = array();

                foreach ($this->join[$this->column]['rel_name'] as $tmp)
                {
                    $tmp_fields[] = '`' . $tmp . '`' . "\r\n";
                }
                // multiselect relation
                if ($this->join[$this->column]['multi'])
                {
                    $where = '`' . $this->join[$this->column]['rel_tbl'] . '`.`' . $this->join[$this->column]['rel_field'] . '` LIKE `' . $this->
                        join[$this->column]['table'] . '`.`' . $this->join[$this->column]['field'] . '`';
                }
                else
                {
                    $where = '`' . $this->join[$this->column]['rel_tbl'] . '`.`' . $this->join[$this->column]['rel_field'] . '` = `' . $this->
                        join[$this->column]['table'] . '`.`' . $this->join[$this->column]['field'] . '`';
                }
                $select = "(SELECT GROUP_CONCAT(DISTINCT (CONCAT_WS('{$this->join[$this->column]['rel_separator']}'," . implode(',', $tmp_fields) .
                    ")) SEPARATOR ', ') 
                            FROM `{$this->join[$this->column]['rel_tbl']}` 
                            WHERE {$where})\r\n";
            }
            // multiselect relation
            elseif ($this->join[$this->column]['multi'])
            {
                $select = "(SELECT GROUP_CONCAT(DISTINCT `{$this->join[$this->column]['rel_name']}` SEPARATOR ', ') 
                        FROM `{$this->join[$this->column]['rel_tbl']}` WHERE 
                        FIND_IN_SET(`{$this->join[$this->column]['rel_tbl']}`.`{$this->join[$this->column]['rel_field']}`,`{$this->join[$this->column]['table']}`.`{$this->join[$this->column]['field']}`) 
                        ORDER BY `{$this->join[$this->column]['rel_name']}` ASC)\r\n";
            }
            else
            {
                $select = "(SELECT `{$this->join[$this->column]['rel_alias']}`.`{$this->join[$this->column]['rel_name']}` 
                            FROM `{$this->join[$this->column]['rel_tbl']}` AS `{$this->join[$this->column]['rel_alias']}` 
                            WHERE `{$this->join[$this->column]['rel_alias']}`.`{$this->join[$this->column]['rel_field']}` = `{$this->join[$this->column]['table']}`.`{$this->join[$this->column]['field']}` 
                            LIMIT 1) \r\n";
            }
        ...
        {
            $or_where = array();
            foreach ($this->join as $column => $param)
            {
                if (is_array($this->join[$column]['rel_name']))
                {
                    ...
                    // multiselect relation
                    if ($this->join[$column]['multi'])
                    {
                        $where = '`' . $this->join[$column]['rel_tbl'] . '`.`' . $this->join[$column]['rel_field'] . '` LIKE `' . $this->join[$column]['table'] .
                            '`.`' . $this->join[$column]['field'] . '`';
                    }
                    else
                    {
                        $where = '`' . $this->join[$column]['rel_tbl'] . '`.`' . $this->join[$column]['rel_field'] . '` = `' . $this->join[$column]['table'] .
                            '`.`' . $this->join[$column]['field'] . '`';
                    }
        ...
        if (isset($this->join[$alias]))
        {
            if ($this->join[$alias]['rel_where'])
            {
                ...
                else
                {
                    ...
                    {
                        $where_arr[] = $this->_field_from_where($field) . $this->_cond_from_where($field) . $db->escape($value);
                    }
    ...
    protected function _receive_post($task = false, $primary = false)
    {
        ...
        else
        {
            ...
            $this->column = $this->_post('column', null, 'key');
    ...
    protected function _build_order_by()
    {
        if (count($this->order_by))
        {
            ...
            {
                if (isset($this->subselect[$field]) or isset($this->columns[$field]) or isset($this->no_select[$field]) or !strpos($field,
                    '.'))
                {
                ...
                foreach ($types as $row)
                {
                    ...
                    if ($row['Key'] == 'PRI' && $row['Extra'] == 'auto_increment')
                    {
                        ...
                        else
                        {
                            ...
                            /*$fields[$field_index] = isset($fields_object[$field_index]) ? $fields_object[$field_index] : array(
                            'table' => $table,
                            'field' => $row['Field'],
                            'tab' => '');*/
                        }
                    ...
                    if ($this->table_join && isset($this->table_join[$table]) && $this->table_join[$table]['join_field'] == $row['Field'])
                    {
                        if (!isset($this->field_type[$field_index]))
                        {
                            $this->_define_field_type($row, $field_index);
    ...
    protected function _define_field_type($row, $field_index)
    {
        ...
        switch ($type)
        {
            ...
            case 'point':
                ...
                if (!isset($this->defaults[$field_index]))
                    $this->defaults[$field_index] = Xcrud_config::$default_point;
                break;
        }
    }
    ...
    protected function _render_list()
    {
        ...
        if (!$this->column && !Xcrud_config::$search_all)
        {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        $this->fields_output[] = array(
                            'label' => $this->fields_names[$field],
                            'field' => call_user_func_array(array($this, $func), array($field, $this->result_row[$field])),
                            'name' => $field,
                            'value' => $this->result_row[$field]);
                    }
                }
            ...
            foreach ($this->inner_table_instance as $inst_name => $field)
            {
                if (isset($this->result_row[$field]))
                {
                    ...
                    $this->data .= '<div class="xcrud-nested-container xcrud-container"><div class="xcrud-ajax" id="xcrud-ajax-' .
                        base_convert(rand(), 10, 32) . '">' . $instance->render('list') . '</div></div>';
                }
            }
        }
        return $this->render_output();
    ...
    {
        $len = isset($this->column_cut_list[$field]) ? $this->column_cut_list[$field] : $this->column_cut;
        if (!$len)
        {
            return $string;
        }
    ...
    public function _export_vars()
    {
        ...
        if ($this->instance_count == 1 && !$this->ajax_request)
        {
            if (isset($_SESSION['xcrud_session']) && $_SESSION['xcrud_session'])
            {
                ...
                { // workaround on some servers session duplication
                    ...
                    if ($time > $old_time + 3) // autocleaner
        ...
        if (Xcrud_config::$alt_session)
        {
            ...
            if (class_exists('Memcache'))
            {
                ...
                $res = $mc->set($sess_id, $data, false, Xcrud_config::$alt_lifetime * 60);
            }
            elseif (class_exists('Memcached'))
            {
                $mc = new Memcached();
                $mc->connect(Xcrud_config::$mc_host, Xcrud_config::$mc_port);
                $res = $mc->set($sess_id, $data, Xcrud_config::$alt_lifetime * 60);
            }
            else
            {
    ...
    protected function create_none($name, $value = '')
    {
    ...
    protected function create_view_bool($name, $value = '')
    {
    ...
    protected function create_view_int($name, $value = '')
    {
        return (int)$value;
    }
    ...
    protected function create_view_float($name, $value = '')
    {
        return (float)$value;
    }
    protected function create_price($name, $value = '')
    {
        ...
        $tag = array(
            ...
            'class' => 'xcrud-input',
            'data-type' => 'price',
            'value' => number_format($value, $this->field_attr[$name]['decimals'], '.', ''),
            'name' => $name,
            'data-pattern' => 'numeric');
        if (isset($this->validation_required[$name]))
        {
    ...
    protected function create_view_price($name, $value = '')
    {
        $out = '';
        $out .= $this->html_safe($this->field_attr[$name]['prefix']);
        $out .= number_format($value, $this->field_attr[$name]['decimals'], '.', $this->html_safe($this->field_attr[$name]['separator']));
    ...
    protected function create_view_text($name, $value = '')
    {
    ...
    protected function create_view_textarea($name, $value = '')
    {
        return $value;
    }
    protected function create_texteditor($name, $value = '')
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_texteditor($name, $value);
        }
        $tag = array(
            'tag' => 'textarea',
            'class' => 'xcrud-input xcrud-texteditor',
            'data-type' => 'texteditor',
            'name' => $name,
            'id' => 'editor-' . base_convert(rand(), 10, 36));
        if (isset($this->validation_required[$name]))
        {
            $tag['data-required'] = $this->validation_required[$name];
        }
    ...
    protected function create_view_texteditor($name, $value = '')
    {
        return $value;
    }
    protected function create_date($name, $value = '')
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_date($name, $value);
        }
        $tag = array(
            'tag' => 'input',
            'type' => 'text',
            'class' => 'xcrud-input xcrud-datepicker',
            'data-type' => 'date',
    ...
    protected function create_view_date($name, $value = '')
    {
    ...
    protected function create_datetime($name, $value = '')
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_datetime($name, $value);
        }
        $tag = array(
            'tag' => 'input',
            'type' => 'text',
    ...
    protected function create_view_datetime($name, $value = '')
    {
    ...
    protected function create_timestamp($name, $value = '')
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_timestamp($name, $value);
        }
        $tag = array(
            'tag' => 'input',
            'type' => 'text',
    ...
    protected function create_view_timestamp($name, $value = '')
    {
        return $this->mysql2datetime($value);
    }
    protected function create_time($name, $value = '')
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_time($name, $value);
        }
        $tag = array(
            'tag' => 'input',
            'type' => 'text',
    ...
    protected function create_view_time($name, $value = '')
    {
    ...
    protected function create_year($name, $value = '')
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_year($name, $value);
        }
        $tag = array(
            'tag' => 'input',
            'type' => 'text',
    ...
    protected function create_view_year($name, $value = '')
    {
        return $value;
    }
    protected function create_select($name, $value = '')
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_select($name, $value);
        }
        $out = '';
        $tag = array(
            'tag' => 'select',
        ...
        {
            $tmp = explode(',', $this->field_attr[$name]['values']);
            foreach ($tmp as $opt)
            {
                $opt = trim(trim($opt, '\''));
    ...
    protected function create_radio($name, $value = '')
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_radio($name, $value);
        }
        $out = '';
        $tag = array(
            'tag' => 'input',
        ...
        if (is_array($this->field_attr[$name]['values']))
        {
            foreach ($this->field_attr[$name]['values'] as $optkey => $opt)
            {
                ...
                $attr = array('value' => $this->html_safe($optkey));
                if ($optkey == $value)
                {
                    $attr['checked'] = '';
                }
        ...
        else
        {
            ...
            foreach ($tmp as $opt)
            {
                $opt = trim(trim($opt, '\''));
                $out .= $this->open_tag($label_tag);
                $attr = array('value' => $this->html_safe($opt));
                if ($opt == $value)
                {
                    $attr['checked'] = '';
                }
    ...
    protected function create_view_select($name, $value = '')
    {
        if (is_array($this->field_attr[$name]['values']) && isset($this->field_attr[$name]['values'][$value]))
        {
    ...
    protected function create_view_radio($name, $value = '')
    {
        return $this->create_view_select($name, $value);
    }
    protected function create_multiselect($name, $value = '')
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_multiselect($name, $value);
        }
        $out = '';
    ...
    protected function create_checkboxes($name, $value = '')
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_checkboxes($name, $value);
        }
        $out = '';
        $values = explode(',', $value);
        $tag = array(
    ...
    protected function create_view_multiselect($name, $value = '')
    {
        $out = array();
        $values = explode(',', $value);
        foreach ($values as $val)
        {
            if (is_array($this->field_attr[$name]['values']) && isset($this->field_attr[$name]['values'][$val]))
            {
    ...
    protected function create_view_checkboxes($name, $value = '')
    {
        return $this->create_view_multiselect($name, $value);
    }
    protected function create_hidden($name, $value = '')
    {
        return $this->single_tag(array(
            'tag' => 'input',
            'type' => 'hidden',
            'value' => $this->html_safe($value),
            'name' => $name,
            'class' => 'xcrud-input'), '', $this->field_attr[$name], true);
    }
    protected function create_password($name, $value = '')
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_password($name, $value);
        }
        $tag = array(
            'tag' => 'input',
            'type' => 'password',
    ...
    protected function create_view_password($name, $value = '')
    {
        return '***';
    }
    protected function create_relation($name, $value = '', $dependval = false)
    {
        ...
        if ($this->join[$name]['depend_on'])
        {
            $tag['data-depend'] = $this->join[$name]['depend_on'];
        }
        ...
        if ($this->join[$name]['rel_where'])
        {
            ...
            {
                $where_arr[] = $this->replace_text_variables($this->join[$name]['rel_where'], $this->result_row);
            }
        }
        if ($dependval !== false)
        {
            $where_arr[] = $this->_field_from_where($this->join[$name]['depend_field']) . $this->_cond_from_where($this->join[$name]['depend_field']) .
                $db->escape($dependval);
        }
        $out .= $this->open_tag($tag, '', array(), true);
        ...
        else
        {
            if ($where_arr)
                ...
                $where = '';
            if (is_array($this->join[$name]['rel_name']))
            {
                $name_select = 'CONCAT_WS(' . $db->escape($this->join[$name]['rel_separator']) . ',`' . implode('`,`', $this->join[$name]['rel_name']) .
                    '`) AS `name`';
            }
            else
            {
                $name_select = '`' . $this->join[$name]['rel_name'] . '` AS `name`';
            }
            $db->query('SELECT `' . $this->join[$name]['rel_field'] . '` AS `field`,' . $name_select . $this->
                get_relation_tree_fields($this->join[$name]) . ' FROM `' . $this->join[$name]['rel_tbl'] . '` ' . $where .
                ' GROUP BY `field` ORDER BY ' . $this->get_relation_ordering($this->join[$name]));
            $options = $this->resort_relation_opts($db->result(), $this->join[$name]);
            if (Xcrud_config::$lists_null_opt)
            {
        ...
        if ($options)
        {
            ...
            {
                $opt_tag = array('tag' => 'option', 'value' => $this->html_safe($opt['field']));
    ...
    protected function create_view_relation($name, $value = '', $dependval = false)
    {
        $db = Xcrud_db::get_instance($this->connection);
        if (is_array($this->join[$name]['rel_name']))
        {
            $field = 'CONCAT_WS(' . $db->escape($this->join[$name]['rel_separator']) . ',`' . implode('`,`', $this->join[$name]['rel_name']) .
                '`) as `name`';
        }
        else
        {
            $field = '`' . $this->join[$name]['rel_name'] . '` as `name`';
        }
        if ($this->join[$name]['multi'])
        {
            $values = explode(',', $value);
        ...
        $db->query('SELECT ' . $field . ' FROM `' . $this->join[$name]['rel_tbl'] . '` WHERE `' . $this->join[$name]['rel_field'] .
            '` ' . $where . ' GROUP BY `' . $this->join[$name]['rel_field'] . '`');
        $options = $db->result();
        $out = array();
        foreach ($options as $opt)
        {
    ...
    protected function create_file($name, $value = '')
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_file($name, $value, $generate_key);
        }
        ...
        if ($value)
        {
            ...
            $out .= $this->lang('remove') . $this->close_tag('a');
        }
        else
        {
        ...
        $attr = array(
            ...
            'class' => 'xcrud-upload',
            'name' => 'xcrud-attach');
        if (isset($this->validation_required[$name]) && !$value)
        {
            $attr['required'] = '';
        }
    ...
    protected function create_view_file($name, $value = '', $generate_key = false)
    {
        $out = '';
        if ($value)
        {
    ...
    protected function create_image($name, $value = '')
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_image($name, $value);
        }
        $out = ''; // upload container
        ...
        if ($value)
        {
            ...
            $attr = array('src' => $this->file_link($name, $this->primary_val), 'alt' => '');
    ...
    protected function create_view_image($name, $value = '')
    {
    ...
    protected function create_view_binary($name, $value = '')
    {
        return $value ? '[binary data]' : '[binary null]';
    }
    protected function create_remote_image($name, $value = '')
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_remote_image($name, $value);
        }
        $out = '';
    ...
    protected function create_view_remote_image($name, $value = '')
    {
        if ($value)
        {
            $attr = $this->field_attr[$name];
    ...
    protected function create_point($name, $value = '')
    {
        if (isset($this->subselect[$name]))
        {
            return $this->create_view_point($name, $value);
        }
        $out = '';
        $attr = $this->field_attr[$name];
        if (!$value)
    ...
    protected function create_view_point($name, $value = '')
    {
        $out = '';
        $attr = $this->field_attr[$name];
        if (!$value)
                ...
                if ($fk['rel_where'])
                {
                    if (is_array($params['rel_where']))
                    {
                        foreach ($params['rel_where'] as $field => $val)
                        {
                            ...
                            {
                                $where_arr[] = "`a`." . $this->_field_from_where($field) . $this->_cond_from_where($field) . $db->escape($val);
                            }
                            else
                            {
                                $where_arr[] = $field;
    ...
    protected static function error($text = 'Error!')
    {
        exit('<div class="xcrud-error" style="position:relative;line-height:1.25;padding:15px;color:#BA0303;margin:10px;border:1px solid #BA0303;border-radius:4px;font-family:Arial,sans-serif;background:#FFB5B5;box-shadow:inset 0 0 80px #E58989;">
            <span style="position:absolute;font-size:10px;bottom:3px;right:5px;">Team P-Wave</span>' . $text . '</div>');
    }
    ...
    protected function _upload_file()
    {
        ...
        if (isset($_FILES) && isset($_FILES['xcrud-attach']) && !$_FILES['xcrud-attach']['error'])
        {
            ...
            $out = $this->create_file($field, $filename);
            if ($this->after_upload)
            {
                $path = $this->check_file($this->after_upload['path'], 'after_upload');
                include_once ($path);
        ...
        if (isset($_FILES) && isset($_FILES['xcrud-attach']) && !$_FILES['xcrud-attach']['error'])
        {
            ...
            if ($this->is_resize($field))
            {
                ...
                else
                {
                    ...
                    $out = $this->create_image($field, $filename);
                }
            }
            else
            {
    ...
    protected function create_modal($field, $content, $image = false)
    {
        ...
        $attr = array(
            'href' => 'javascript:;',
            'data-header' => $this->html_safe($this->columns_names[$field]),
            'data-content' => $this->html_safe($content));
        if ($image)
        {
            $attr['data-content'] = $this->html_safe($this->single_tag('img', '', array('alt' => '', 'src' => $image)));
        }
        else
        {
            $attr['data-content'] = $this->html_safe($content);
        }
    ...
    protected function _render_list_item($field, $value, $primary_val, $row)
    {
        ...
        if (isset($this->field_type[$field]))
        {
            ...
            {
                case 'select':
                    if (isset($this->field_attr[$field]['values'][$value]) && is_array($this->field_attr[$field]['values']))
                    {
                        $out .= $this->_cut($this->field_attr[$field]['values'][$value], $field);
                    }
                    else
                    {
                ...
                case 'price':
                    ...
                    $out .= number_format((float)$value, (int)$this->field_attr[$field]['decimals'], '.', $this->field_attr[$field]['separator']);
    ...
    protected function make_mailto($txt)
    {
        if ($this->emails_label)
            return preg_replace('/([A-Za-z0-9_\-\.]+)\@([A-Za-z0-9_\-\.]+)\.([A-Za-z]{2,4})/', '<a href="mailto:$1@$2.$3">' . $this->
                emails_label['text'] . '</a>', $txt);
        else
            return preg_replace('/([A-Za-z0-9_\-\.]+)\@([A-Za-z0-9_\-\.]+)\.([A-Za-z]{2,4})/',
                '<a href="mailto:$1@$2.$3">$1@$2.$3</a>', $txt);
    }
    protected function make_links($txt)
    {
        if ($this->links_label)
            return preg_replace('/(http:\/\/|https:\/\/)([^\s]+)/u', '<a href="$1$2">' . $this->links_label['text'] . '</a>', $txt);
        else
            return preg_replace('/(http:\/\/|https:\/\/)([^\s]+)/u', '<a href="$1$2">$1$2</a>', $txt);
    }
    ...
    protected function _render_export_item($field, $value, $primary_val, $row)
    {
        ...
        if (isset($this->column_pattern[$field]))
        {
            $value = strip_tags($this->replace_text_variables($this->column_pattern[$field], $row, true));
        }
    ...
    protected function _render_list_buttons(&$row)
    {
            ...
            foreach ($this->buttons as $button)
            {
                if ($this->is_button($button['name'], $row))
                {
                    ...
                    $tag = array(
                        ...
                        'href' => $link,
                        'title' => $this->html_safe($button['name']));
        ...
        if (!isset($this->hide_button['remove']) && !$this->table_ro && $this->is_remove($row))
        {
            ...
            {
                $tag['data-confirm'] = $this->html_safe($this->lang('deleting_confirm'));
            }
    ...
    protected function _check_unique_value()
    {
        ...
        foreach ($fdata as $fitem)
        {
            ...
            {
                $out[] = '[name="' . $fitem['table'] . '.' . $fitem['field'] . '"]';
            }
        }
        if ($out)
        {
    ...
    protected function _check_url($url, $scr_url = false)
    {
        if (!$url && !$scr_url)
            return false;
        $url = rtrim($url, '/');
    ...
    protected function _clone_row()
    {
        if (is_array($this->table_info) && count($this->table_info) && !$this->table_ro)
        {
            ...
            if ($fields)
            {
                if (!$this->table_join)
                {
                    $db->query('SELECT ' . implode(',', $fields) . " FROM `{$this->table}` WHERE `{$this->primary_key}` = " . $db->escape($primary) .
                        " LIMIT 1");
                    $row = $db->row();
            ...
            if (isset($this->pass_var['create']))
            {
                foreach ($this->pass_var['create'] as $field => $value)
                {
    ...
    public function _parse_field_names($fields = '', $location = '')
    {
        $field_names = array();
        if ($fields)
        {
            ...
            if (is_array($fields))
            {
                foreach ($fields as $key => $val)
                {
                    ...
                    {
                        if (!strpos($val, '.'))
                            $field_names[$table . '.' . $val] = array('table' => $table, 'field' => $val);
                        else
                        {
                            $tmp = explode('.', $val);
                            $field_names[$tmp[0] . '.' . $tmp[1]] = array('table' => $tmp[0], 'field' => $tmp[1]);
                            unset($tmp);
                        }
                    }
                    else
    ...
    public static function load_js()
    {
        ...
        if (Xcrud_config::$load_googlemap)
            $out .= '<script src="http://maps.google.com/maps/api/js?sensor=false&language=' . $language . '"></script>';
    ...
    protected function get_limit_list($limit = 20, $buttons = false)
    {
        ...
        if ($buttons)
        {
            ...
            foreach ($this->limit_list as $limts)
            {
                ...
                $out .= $this->open_tag(array(
                    ...
                    'data-limit' => $limts), 'xcrud-action') . $limts . $this->close_tag(array('tag' => 'button'));
            }
        ...
        else
        {
            ...
            foreach ($this->limit_list as $limts)
            {
                ...
                $out .= $this->open_tag($tag) . $limts . $this->close_tag($tag);
            }
    ...
    protected function additional_columns($fields = '')
    {
        if ($fields)
        {
            ...
            foreach ($fdata as $fitem)
            {
                ...
                if (!isset($this->columns[$key]) && !isset($this->subselect[$key]) && $fitem['field'] != 'value')
                {
                    $this->hidden_columns[$key] = $fitem;
                }
                $this->direct_select_tags[$key] = array('field' => $fitem['field'], 'table' => $fitem['table']);
            }
        }
    }
    ...
    {
        if (!is_array($value) && !Xcrud_config::$performance_mode)
        {
    ...
    protected function open_tag($tag = '', $class = '', $attr = array(), $enc_name = false)
    {
        if ($tag)
        {
            ...
            if (isset($tag['tag']))
            {
                ...
                {
                    $tag['name'] = str_replace(array('=', '/'), array('-', '_'), base64_encode($tag['name']));
                }
                if ($enc_name && isset($tag['data-depend']))
                {
                    $tag['data-depend'] = str_replace(array('=', '/'), array('-', '_'), base64_encode($tag['data-depend']));
                }
                if ($tag)
                {
        ...
        'div', $tabs_head = 'ul', $tabs_row = 'li', $tabs_link = 'a', $tabs_content = 'div', $tabs_pane = 'div')
    {
        ...
        if (isset($this->field_tabs[$mode]))
        {
            ...
            foreach ($this->field_tabs[$mode] as $key => $tabname)
            {
                ...
                $tabs_header .= $this->open_tag($tabs_row, $this->theme_config('tabs_content') . ($k == 0 ? ' ' . $this->theme_config('tabs_first_element') :
                    '')) . $this->open_tag($tabs_link, $this->theme_config('tabs_header_link') . ($k == 0 ? ' ' . $this->theme_config('tabs_first_element') :
                    ''), array('href' => '#' . $tabid)) . $tabname . $this->close_tag($tabs_link) . $this->close_tag($tabs_row);
    ...
    protected function get_table_tooltip()
    {
        ...
        {
            $out .= ' <a href="javascript:;" class="xcrud-tooltip' . ' xcrud-button-link" title="' . $this->table_tooltip['tooltip'] .
                '"><i class="' . ($this->table_tooltip['icon'] ? $this->table_tooltip['icon'] : $this->theme_config('tooltip_icon')) .
                '"></i></a>';
        }
        return $out;
    }
    ...
    protected function get_field_tooltip($field, $mode)
    {
        ...
        {
            $out .= ' <a href="javascript:;" class="xcrud-tooltip' . ' xcrud-button-link" title="' . $this->field_tooltip[$field]['tooltip'] .
                '"><i class="' . ($this->field_tooltip[$field]['icon'] ? $this->field_tooltip[$field]['icon'] : $this->theme_config('tooltip_icon')) .
                '"></i></a>';
        }
        return $out;
    }
    ...
    protected function get_column_tooltip($field)
    {
        ...
        {
            $out .= ' <a href="javascript:;" class="xcrud-tooltip' . ' xcrud-button-link" title="' . $this->column_tooltip[$field]['tooltip'] .
                '"><i class="' . ($this->column_tooltip[$field]['icon'] ? $this->column_tooltip[$field]['icon'] : $this->theme_config('tooltip_icon')) .
                '"></i></a>';
        }
        return $out;
    }
        ...
        if ($this->is_search)
        {
            if (is_array($this->phrase) && isset($this->field_type[$this->column]))
            {
                switch ($this->field_type[$this->column])
                {
                    case 'timestamp':
                    case 'datetime':
                        $phrase = array('from' => $this->unix2datetime((int)$this->phrase['from']), 'to' => $this->unix2datetime((int)$this->
                                phrase['to']));
                        $is_daterange = true;
                        break;
                    case 'date':
                        $phrase = array('from' => $this->unix2date((int)$this->phrase['from']), 'to' => $this->unix2date((int)$this->phrase['to']));
                        $is_daterange = true;
                        break;
                    case 'time':
                        $phrase = array('from' => $this->unix2time((int)$this->phrase['from']), 'to' => $this->unix2time((int)$this->phrase['to']));
                        break;
                    default:
                        $phrase = '';
                        break;
    ...
    protected function search_fieldlist($field, $phrase, $fieldlist)
    {
        ...
        switch ($attr['data-type'])
        {
            ...
            case 'checkboxes':
                ...
                {
                    $opts = explode(',', $this->field_attr[$field]['values']);
                    foreach ($opts as $opt)
                    {
                        $opt = trim($opt, '\'');
    ...
    protected function render_control_fields()
    {
        ...
        if ($this->message)
        {
            $tag['class'] = 'xcrud-callback-message';
            $tag['name'] = $this->message['type'];
            $tag['value'] = $this->html_safe($this->message['text']);
    ...
    protected function remove_tmp_image($filename, $field)
    {
        ...
        if (isset($this->upload_config[$field]['save_original']) && $this->upload_config[$field]['save_original'] == true)
        {
            ...
            rename($this->get_image_folder($field) . '/' . $tmp_filename, get_image_folder($field) . '/' . $orig_filename);
        }
        else
        {
            @unlink($this->get_image_folder($field) . '/' . $tmp_filename);
        }
    }
    ...
    protected function unix2date($time)
    {
        if ($time)
            return date(Xcrud_config::$php_date_format, $time);
        else
            return '';
    }
    protected function unix2datetime($time)
    {
        if ($time)
            return date(Xcrud_config::$php_date_format . ' ' . Xcrud_config::$php_time_format, $time);
        else
            return '';
    }
    protected function unix2time($time)
    {
        if ($time)
            return date(Xcrud_config::$php_time_format, $time);
        else
            return '';
    }
    ...
    protected function render_table_name($mode = 'list', $tag = 'h2', $to_show = false)
    {
        ...
        if ($this->is_title)
        {
            ...
            {
                case 'create':
                    $out .= '<small> - ' . $this->lang('add') . '</small>';
                    break;
                case 'edit':
                    $out .= '<small> - ' . $this->lang('edit') . '</small>';
                    break;
                case 'view':
                    $out .= '<small> - ' . $this->lang('view') . '</small>';
                    break;
                default:
                    $out .= '<small> ' . $this->get_table_tooltip() . '</small>';
                    break;
            }
            if ($to_show)
...
class Xcrud_postdata
{
    ...
    public function get($name)
    {
        $fdata = $this->xcrud->_parse_field_names($name, 'Xcrud_postdata');
        $fname = key($fdata) /*$fdata[0]['table'] . '.' . $fdata[0]['field']*/;
        return (isset($this->postdata[$fname]) ? $this->postdata[$fname] : false);
    }
